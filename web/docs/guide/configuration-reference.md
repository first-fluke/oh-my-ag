...
| `providers` | `docs`, `web`, `code_intelligence`, `semantic_memory` | `context7`, `native`, `serena`, `agentmemory` | Select documentation, search, code-intelligence, and semantic-memory providers. Code intelligence accepts `serena` or `gortex`; semantic memory accepts `agentmemory`, `honcho`, or `none`. |
