...
oh-my-agent은 멀티 에이전트 워크플로우 중 에이전트 활동을 모니터링하기 위한 두 가지 실시간 대시보드를 제공합니다.
...
- [ ] **모든 에이전트가 "completed"로 표시**: "running"이나 "blocked" 상태인 에이전트가 없습니다.
- [ ] **"failed"로 표시된 에이전트 없음**: 실패한 에이전트가 있으면 로그를 확인하고 다시 생성합니다.
- [ ] **QA 에이전트가 리뷰를 완료**: `result-qa-agent.md` 또는 `result-qa.md`를 확인합니다.
- [ ] **CRITICAL/HIGH 발견 사항 0건**: QA 결과 파일에서 심각도 건수를 확인합니다.
- [ ] **세션 상태가 COMPLETED**: 세션 파일이 최종 상태를 표시해야 합니다.
- [ ] **활동 피드에 최종 보고서 표시**: 마지막 활동이 요약 보고서여야 합니다.
...
1. 생성된 에이전트가 모두 최종 상태에 도달했습니다 (완료했거나, 실패한 뒤 처리했습니다).
2. QA 리뷰 사이클이 차단 이슈 없이 끝났습니다.
3. 세션 상태가 최종 결과를 반영합니다.
4. 나중에 참고할 수 있도록 결과를 메모리에 기록했습니다.
