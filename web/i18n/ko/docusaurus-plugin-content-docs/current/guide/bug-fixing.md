...
title: "가이드: 버그 수정"
sidebar_label: 버그 수정
description: 심각도 분류, 에스컬레이션 시그널, 소스에 근거한 진단, 수정 후 검증을 포함하는 7단계 디버깅 워크플로우입니다.
---
...
디버그 워크플로우는 설정된 모든 벤더를 지원합니다. 1-5단계는 인라인으로 실행됩니다. 6단계(유사 패턴 스캔)는 스캔 범위가 넓을 때(10개 이상 파일 또는 여러 도메인에 걸친 오류) `debug-investigator` 서브에이전트에 위임할 수 있으며, 이어서 7단계에서 메모리에 기록합니다.

---

## 버그 리포트 템플릿

버그를 보고할 때 가능한 한 다음 정보를 제공하세요. 각 필드는 디버그 워크플로우가 검색 범위를 더 빠르게 좁히는 데 도움이 됩니다.
...
| **재현 단계** | 버그를 트리거하는 동작 순서 | 1. 관리자로 로그인. 2. /users로 이동. 3. 아무 사용자에서 "삭제" 클릭. |
...
**디버그 접근:** 전체 7단계 루프를 실행합니다. 수정 후 QA 리뷰를 권장합니다.
...
**디버그 접근:** 전체 7단계 루프를 실행합니다. QA 회귀 테스트 스위트에 포함합니다.
...
`/debug` 워크플로우는 이 단계들을 순서대로 실행합니다. 설정된 코드 인텔리전스 provider를 사용할 수 있으면 우선 사용하고, 해당 provider를 사용할 수 없거나 시간 초과되면 native 검색과 범위를 제한한 파일 읽기를 함께 사용합니다.

### Step 1: 에러 정보 수집

워크플로우가 요청하는(또는 사용자에게서 받는) 정보:
- 에러 메시지와 스택 트레이스
- 재현 단계
- 예상 vs 실제 동작
- 환경 세부사항
...
**사용 도구:** 설정된 검색·심볼 도구를 사용합니다. 설정된 도구를 사용할 수 없으면 native `rg`와 범위를 제한한 파일 읽기를 사용합니다.

목표는 코드베이스에서 에러의 위치를 찾는 것입니다. 예외가 발생하는 정확한 라인, 잘못된 출력을 생성하는 정확한 함수, 예상치 못한 동작을 유발하는 정확한 조건을 찾습니다.

이 단계는 사용자가 보고한 증상("페이지가 크래시됨")을 코드베이스 수준 위치(`src/api/users.ts:47, deleteUser()에서 TypeError 발생`)로 변환합니다.

### Step 3: 근본 원인 진단

**사용 도구:** 참조와 심볼 탐색을 사용할 수 있으면 우선 사용하고, 그렇지 않으면 필요한 범위의 native 읽기를 이어서 사용합니다.
...
| **레이스 컨디션** | 순서 없이 완료되는 비동기 연산, 누락된 await, 공유 가변 상태 |
...
증상이 아니라 근본 원인을 진단합니다. `user.id`가 undefined라면 "undefined를 어떻게 막을까"가 아니라 "이 실행 경로에서 user가 왜 undefined인가"를 먼저 물어야 합니다.
...
워크플로우는 편집 전에 제안 내용을 보여 줍니다. 요청이나 실행 정책으로 이미 권한이 부여되지 않은 변경이면 확인을 기다리며, 이미 권한이 있으면 다시 묻지 않고 진행합니다.
...
2. **회귀 테스트 작성**: 다음 조건을 만족하는 테스트를 만듭니다
...
**사용 도구:** 설정된 패턴 검색을 사용하거나, 근본 원인으로 확인된 패턴을 native 검색으로 범위를 제한해 찾습니다.
...
프론트엔드의 에러가 백엔드 변경 때문에 발생하고, 그 변경은 다시 데이터베이스 스키마 마이그레이션에서 비롯된 경우입니다. 근본 원인이 도메인 경계를 넘을 때는 관련 도메인 에이전트를 포함하도록 `/work` 또는 `/orchestrate`로 에스컬레이션합니다.

### 시그널 3: 재현 환경 부재

버그가 프로덕션에서만 발생하고 로컬에서는 재현되지 않는 경우입니다. 다음과 같은 신호가 있습니다.
- 환경별 설정 차이.
- 프로덕션 부하에서만 드러나는 레이스 컨디션.
- 스테이징과 프로덕션 사이의 서드파티 서비스 동작 차이.

**조치:** 프로덕션 로그를 수집하고, 프로덕션 모니터링 접근을 요청하며, 수정을 시도하기 전에 계측과 로깅 추가를 고려합니다.

### 시그널 4: 테스트 인프라 장애

테스트 인프라가 깨졌거나, 없거나, 부적절해서 회귀 테스트를 작성할 수 없는 경우입니다.

**조치:** 먼저 테스트 인프라를 수정한 다음(또는 `oma install`로 설정) 디버그 워크플로우로 돌아옵니다. 실행 가능한 검사가 적용되지 않으면 결과 컨트랙트에 그 이유를 기록하고, 통과한 것처럼 보이는 검사를 만들지 않습니다.
...
- [ ] **수정 없이 회귀 테스트가 실패**: 수정을 잠시 되돌려 테스트가 버그를 잡는지 확인합니다.
- [ ] **수정 적용 시 회귀 테스트가 통과**: 수정을 적용하고 테스트가 통과하는지 확인합니다.
- [ ] **관련 기존 검사가 여전히 통과**: 변경된 동작을 다루는 프로젝트 검사를 실행합니다. 빌드는 태스크에서 명시적으로 요구할 때만 실행합니다.
- [ ] **유사 패턴 스캔 완료**: Step 6을 마쳤고, 발견한 인스턴스를 모두 수정하거나 문서로 남겼습니다.
- [ ] **수정이 최소한**: 필요한 줄만 바꿨고, 관련 없는 리팩토링은 넣지 않았습니다.
- [ ] **근본 원인 문서화**: 메모리 파일에 기록합니다 (증상, 근본 원인, 적용된 수정, 변경된 파일, 회귀 테스트 위치, 유사 패턴).
...
1. 증상이 아니라 근본 원인을 찾아 문서로 남겼습니다.
2. 태스크에서 부여한 권한에 따라 최소 수정을 적용했습니다.
3. 수정 없이 실패하고 수정을 적용하면 통과하는 회귀 테스트가 있습니다.
4. 같은 패턴을 찾아 코드베이스를 스캔했고, 확인한 인스턴스를 모두 처리했습니다.
5. 메모리에 버그 리포트를 기록했습니다.
6. 수정 후에도 기존 테스트가 모두 통과합니다.
