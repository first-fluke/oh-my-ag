---
title: "指南：Bug 修复"
sidebar_label: Bug 修复
description: 结构化的七阶段调试工作流，涵盖严重度分级、升级信号、基于源代码的诊断和修复后验证。
---
...
调试工作流支持所有已配置的供应商。步骤 1-5 内联运行。步骤 6（扫描类似模式）在扫描范围较广（10+ 个文件或多领域错误）时可能委派给 `debug-investigator` 子智能体，随后执行步骤 7 的内存记录。
...
`/debug` 工作流按严格顺序执行这些阶段。代码智能提供商可用时使用它；不可用或超时时，则结合原生搜索和限定范围的文件读取。
...
**使用的工具：** 配置的搜索和符号工具；配置的工具不可用时，使用原生 `rg` 和限定范围的文件读取。
...
**使用的工具：** 可用时进行引用和符号导航，不可用时再进行定向的原生读取。
...
工作流会在编辑前展示方案。如果变更尚未得到请求或执行策略的授权，工作流会等待确认；已有授权时无需再次询问。
...
**使用的工具：** 配置的模式搜索；不可用时，使用以根因为模式的限定范围原生搜索。
...
**行动：** 先修复测试基础设施（或使用 `oma install` 配置），然后返回调试工作流。如果不适用可执行检查，则在结果契约中记录原因，不要虚构通过的检查。
...
- [ ] **相关现有检查仍通过**：运行覆盖变更行为的项目检查。仅在任务明确要求时运行构建。
...
相关命令、路径和标识符： `src/api/users.ts:47, deleteUser() throws TypeError`、`.claude/agents/debug-investigator.md`、`user.organization.id`。
